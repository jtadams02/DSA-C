...
// This is the CircularDynamicArray class which will be used for heap storage
...
// Now its time for the Heap code
/* HEAP INFO
*  For learning purposes, I will build a max heap then slowly edit it into a min-heap. Hopefully this will be the best way to learn
*  For every node i, other than root A[PARENT(i)] <= A[i]
*/
template <class keytype> // Declare our keytype template
class Heap{
    private: 

        CircularDynamicArray<keytype> arr; // All I know is that I will need to use this for storage so here it is
        int arrLength = 0;
        int heapSize = 0; // This tracks how many
        int height = 0; // Number of edges

        int smallest; // Smallest value in the tree

        int largest; // INDEX of largest value stored in tree
        ...
        // This code is just for learning how heapify-ing works. This is based off the textbook psuedocode and will be adapted in a MIN_HEAPIFY() function later
        void MAX_HEAPIFY(int i){
            // cout << "Current index: " << i << " and current value: " << arr[i] << endl;
            int l = left(i);
            int r = right(i);

            if(l<=this->heapSize){
                if(arr[l] > arr[i]){
                    // If the value of l is inside heapSize and its key is greater than i set the index of the largest to l
                    largest = l;
                } else {
                    largest = i;
                }
                ...
                // If l is not inside the array, or its key is less than i, set i equal to the largest index
                largest = i;
            }

            if(r<=this->heapSize){
                if(arr[r] > arr[largest]){
                    // Same process for right, make sure r is a valid index in the array, and if its key is > than largest (i) then set largest to r. 
                    largest = r;
                }
            }

            if(largest!=i){
                // If largest does not equal i, then i violates the max-heap property. Switch i and the largest index, then heapify again from the new largest index
                keytype tempI = arr[i];
                arr[i] = arr[largest];
                arr[largest] = tempI;
        ...
        // I believe this is a bottom up. It starts at the lowest parental node which would be heapisze/2
        void MIN_HEAPIFY(int i){
            ...
            if(l<=this->heapSize){
                if(arr[l]<arr[i]){
                    // If the left is less than the parent, then switch them
                    smallest = l;
                } else {
                    smallest = i;
                }
            ...
            if(smallest!=i){
                // If the largest does not equal i, then i volates the min-heap property. Switch i and the largest index, then heapiy again from the new smallest index
                keytype tempI = arr[i];
                arr[i] = arr[smallest];
                arr[smallest] = tempI;
        ...
        // BUILD_MAX_HEAP appears to work only with an non-decreasing array. 
        void BUILD_MAX_HEAP(){
        ...
        void HEAP_DECREASE_KEY(int i,keytype key)
        {
            arr[i] = key;
            // cout << "Parent is: " << arr[parent(i)] << " and child is: " << arr[i] << endl;
            while(i>1 && arr[parent(i)] > arr[i]){
                // cout << "Swapping " << arr[parent(i)] << " and " << arr[i] << endl;
                keytype tempI = arr[i];
                arr[i] = arr[parent(i)];
                arr[parent(i)] = tempI;
                i = parent(i);
        ...
        void MIN_HEAP_INSERT(keytype key){
            heapSize = heapSize + 1; 
            // I believe this is the max number in C++
            arr.addEnd(key);
    ...
    public: 
        ...
        Heap() // This constructor is used in the case of like: Heap<int> x;
        {
            CircularDynamicArray<keytype> newArr(1);
        ...
        Heap(keytype k[], int s) // s is the size of k[]. The heap should be constructed using the O(n) bottom up heap building method
        {
            // Assign CDA with size of +1 because 0 index is to remain empty!
            CircularDynamicArray<keytype> newArr(s+1);

            arr = newArr;

            heapSize = s;

            for(int i=1,j=0;i<s+1;i++,j++){
                arr[i] = k[j];
            }
        ...
        Heap(const Heap& otherHeap) {
            CircularDynamicArray<keytype> newArr(otherHeap.heapSize);
            arr = newArr;
            arr = otherHeap.arr;
            this->heapSize = otherHeap.heapSize;
            this->height = otherHeap.height;
            this->arrLength = otherHeap.arrLength;

        }

        Heap& operator=(const Heap& otherHeap) {
            if (this != &otherHeap) {
                arr.clear(); // Maybe we need this
                CircularDynamicArray<keytype> newArr(otherHeap.heapSize);
                arr = newArr;
                arr= otherHeap.arr;
                this->heapSize = otherHeap.heapSize;
        ...
        keytype peekKey() // Returns minimum key in the heap without modifying the heap at all
        {
            if(heapSize>=1){
                return arr[1];
            } else {
        ...
        keytype extractMin() // Removes the minimum key from the heap and returns the key
        {
        ...
        void insert(keytype k) // Inserts k into the heap
        {
            MIN_HEAP_INSERT(k);
        }

        void printKey() // Writes the keys stored in the array, starting at the root
        {
            if(heapSize!=0){
                arr.heapPrint(heapSize);
            }

        }

        keytype getValue(int index){
            return arr[index];
        }
};